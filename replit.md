...
## Overview
Atlas is a desktop-first personal productivity command center designed as a local-first capture intelligence system. It prioritizes cost-effective local parsing for user input, using AI (Claude Haiku) only as a fallback for ambiguity, thus minimizing AI expenses. The application is deployed on a \`.replit.app\` domain, leveraging Neon Postgres for data persistence, Clerk for authentication, and Cloudflare R2 for file storage. Its core purpose is to provide a comprehensive tool for managing tasks, notes, structured data (tables), and projects, with advanced features like full-text search, background job processing, and robust Google Drive integration.
...
- **Modules**:
    ...
    - **File Conversion (Import/Export)**: Notes support file conversion via \`src/core/conversion/\`:
        - **Import**: \`.md\` (Markdown with front-matter via gray-matter + marked) and \`.docx\` (Word via mammoth) files can be imported using the split-button dropdown on the note list view. Imports trigger a progress dialog and optionally a Claude-assisted conflict resolver if a note with the same title already exists.
        - **Export**: Notes can be exported as PDF (pdfkit-based, from the Actions section in the metadata panel) or Markdown (direct download from the same panel). Export API routes: \`POST /api/convert/export-pdf\` and \`POST /api/convert/export-md\`.
        - **Import API**: \`POST /api/convert/import\` (multipart/form-data) handles both formats with per-user rate limiting (10 imports/min, 5 PDF exports/min).
        - **Cleanup job**: \`import-cleanup\` runs daily at 06:00 UTC to purge expired R2 objects from PDF exports.
        - **Note model**: Three fields added: \`imported_from\`, \`imported_at\`, \`source_metadata\`.
...
- **gray-matter, marked**: Markdown front-matter parsing and HTML conversion (import)
- **mammoth**: Word (.docx) to HTML conversion (import)
- **pdfkit**: Pure Node.js PDF generation (export)
