...
export function PersonForm({ mode, personId }: Props) {
  ...
  useEffect(() => {
    ...
    setEmails(
      p.emails.map((e) => ({ id: e.id, email: e.email, type: e.type, is_primary: e.is_primary })),
    );
    setPhones(
      p.phones.map((ph) => ({
        id: ph.id,
    ...
    setOrgs(
      p.organizations.map((o) => ({
        ...
        start_date: o.start_date ? new Date(o.start_date).toISOString().slice(0, 10) : "",
        end_date: o.end_date ? new Date(o.end_date).toISOString().slice(0, 10) : "",
      })),
    );
    ...
    setRelations(
      p.relations.map((r) => ({
        id: r.id,
        related_person_id: r.related_person_id ?? undefined,
        related_text: r.related_text ?? "",
        type: r.type,
        use_picker: !!r.related_person_id,
      })),
    );
    ...
    if (cd === null) {
      ...
      const preset = CADENCE_PRESETS.find(
        (x) => x.value === cd && x.value !== null && x.value !== -1,
      );
      if (preset) {
        setCadencePreset(cd);
      } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const hasAnyName =
      displayName.trim() || givenName.trim() || familyName.trim() || nickname.trim();
    if (!hasAnyName) {
    ...
    try {
      ...
      const removals: Promise<unknown>[] = [
        ...[...origEmailIds]
          .filter((id) => !currEmailIds.has(id))
          .map((id) => removeEmail.mutateAsync({ id })),
        ...[...origPhoneIds]
          .filter((id) => !currPhoneIds.has(id))
          .map((id) => removePhone.mutateAsync({ id })),
        ...[...origAddressIds]
          .filter((id) => !currAddressIds.has(id))
          .map((id) => removeAddress.mutateAsync({ id })),
        ...[...origOrgIds]
          .filter((id) => !currOrgIds.has(id))
          .map((id) => removeOrg.mutateAsync({ id })),
        ...[...origUrlIds]
          .filter((id) => !currUrlIds.has(id))
          .map((id) => removeUrl.mutateAsync({ id })),
        ...[...origEventIds]
          .filter((id) => !currEventIds.has(id))
          .map((id) => removeEvent.mutateAsync({ id })),
        ...[...origRelationIds]
          .filter((id) => !currRelationIds.has(id))
          .map((id) => removeRelation.mutateAsync({ id })),
        ...[...origSkillIds]
          .filter((id) => !currSkillIds.has(id))
          .map((id) => removeSkill.mutateAsync({ id })),
        ...[...origInterestIds]
          .filter((id) => !currInterestIds.has(id))
          .map((id) => removeInterest.mutateAsync({ id })),
      ];
      ...
      for (const r of phones) {
        if (!r.id && r.number.trim())
          upserts.push(
            addPhone.mutateAsync({
              person_id: pid,
      ...
      for (const r of addresses) {
        ...
        if (!r.id && (r.street.trim() || r.city.trim()))
          upserts.push(addAddress.mutateAsync({ person_id: pid, ...addrData }));
      ...
      for (const r of orgs) {
        const orgData = {
          ...
          is_primary: r.is_primary,
          start_date: r.start_date ? toIso(r.start_date) : undefined,
          end_date: r.end_date ? toIso(r.end_date) : undefined,
        };
        if (!r.id && r.name.trim())
          upserts.push(addOrg.mutateAsync({ person_id: pid, ...orgData }));
      ...
      for (const r of relations) {
        ...
        const relData = {
          type: r.type,
          related_person_id: r.related_person_id || undefined,
          related_text: r.related_text || undefined,
        };
      ...
      for (const r of skills) {
        if (!r.id && r.name.trim())
          upserts.push(addSkill.mutateAsync({ person_id: pid, name: r.name.trim() }));
      }
      for (const r of interests) {
        if (!r.id && r.name.trim())
          upserts.push(addInterest.mutateAsync({ person_id: pid, name: r.name.trim() }));
      }
  ...
  const previewName = deriveDisplayName({
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="sticky top-0 z-10 flex items-center gap-2 border-b border-border-subtle bg-surface-base px-4 py-2.5">
          ...
          <span className="flex-1 truncate pl-2 text-sm text-text-primary">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              form="person-form"
              disabled={submitting}
              className="rounded-md bg-accent-primary px-3 py-1.5 text-sm text-text-on-accent transition-colors hover:bg-accent-primary-hover disabled:opacity-60"
            >
        ...
        <form
          ...
          className="mx-auto max-w-2xl space-y-8 px-4 pb-16 pt-6"
        >
          {error && (
          ...
          <section id="section-name">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-text-disabled">
              Name
            </h2>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Display name</label>
                <input
                  className={INPUT}
                  value={displayName}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Nickname</label>
                <input
                  className={INPUT}
                  value={nickname}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Honorific prefix</label>
                <input
                  className={INPUT}
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Given name</label>
                <input
                  className={INPUT}
                  value={givenName}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Middle name</label>
                <input
                  className={INPUT}
                  value={middleName}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Family name</label>
                <input
                  className={INPUT}
                  value={familyName}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Honorific suffix</label>
                <input
                  className={INPUT}
                  value={honorificSuffix}
                  onChange={(e) => {
                    setHonorificSuffix(e.target.value);
                    markDirty();
                  }}
          ...
          <section id="section-profile">
            ...
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Relationship type</label>
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Photo URL</label>
                <input
                  className={INPUT}
                  value={photoUrl}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1 block text-xs text-text-tertiary">Biography</label>
                <textarea
              ...
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    setEmails([
                      ...emails,
                      { email: "", type: "other", is_primary: emails.length === 0 },
                    ]);
                    markDirty();
                  }}
                  className={ADD_BTN}
                ...
                {emails.map((email, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setEmails(setPrimary(emails, idx));
                        markDirty();
                      }}
                      className={cn(
                        "shrink-0",
                    ...
                    <button
                      ...
                      className="shrink-0 text-text-disabled hover:text-accent-danger"
                    >
                      <X size={14} />
                    </button>
                  </div>
              ...
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    setPhones([
                      ...phones,
                      { number: "", type: "mobile", is_primary: phones.length === 0 },
                    ]);
                    markDirty();
                  }}
                  className={ADD_BTN}
                ...
                {phones.map((phone, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setPhones(setPrimary(phones, idx));
                        markDirty();
                      }}
                      className={cn(
                        "shrink-0",
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => {
                    setUrls([...urls, { url: "", type: "other", label: "" }]);
                    markDirty();
                  }}
                  className={ADD_BTN}
                >
          ...
          <section id="section-work">
            ...
            <div className="space-y-4">
              {orgs.map((org, idx) => (
                <div key={idx} className="space-y-2 rounded-lg border border-border-subtle p-3">
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setOrgs(setPrimary(orgs, idx));
                        markDirty();
                      }}
                      className={cn(
                        "shrink-0",
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...orgs];
                        n[idx] = { ...n[idx]!, department: e.target.value };
                        setOrgs(n);
                        markDirty();
                      }}
                  ...
                  <div className="flex items-center gap-3">
                    <label className="flex cursor-pointer items-center gap-1.5 text-xs text-text-secondary">
                      <input
                        type="checkbox"
                        checked={org.is_current}
                        onChange={(e) => {
                          const n = [...orgs];
                          n[idx] = {
                            ...n[idx]!,
                            is_current: e.target.checked,
                            end_date: e.target.checked ? "" : n[idx]!.end_date,
                          };
                          setOrgs(n);
                          markDirty();
                        }}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...orgs];
                        n[idx] = { ...n[idx]!, start_date: e.target.value };
                        setOrgs(n);
                        markDirty();
                      }}
                    ...
                    {!org.is_current && (
                      <input
                        ...
                        onChange={(e) => {
                          const n = [...orgs];
                          n[idx] = { ...n[idx]!, end_date: e.target.value };
                          setOrgs(n);
                          markDirty();
                        }}
                ...
                <div key={idx} className="space-y-2 rounded-lg border border-border-subtle p-3">
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setAddresses(setPrimary(addresses, idx));
                        markDirty();
                      }}
                      className={cn(
                        "shrink-0",
                    ...
                    <button
                      ...
                      className="ml-auto shrink-0 text-text-disabled hover:text-accent-danger"
                    >
                      <X size={14} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...addresses];
                        n[idx] = { ...n[idx]!, region: e.target.value };
                        setAddresses(n);
                        markDirty();
                      }}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...addresses];
                        n[idx] = { ...n[idx]!, postal_code: e.target.value };
                        setAddresses(n);
                        markDirty();
                      }}
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const opt = ISO_COUNTRIES.find((c) => c.code === e.target.value);
                        const n = [...addresses];
                        n[idx] = {
                          ...n[idx]!,
                          country_code: e.target.value,
                          country_name: opt?.name ?? "",
                        };
                        setAddresses(n);
                        markDirty();
                      }}
          ...
          <section id="section-events">
            ...
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => {
                  setEvents([...events, { type: "birthday", date: "", label: "" }]);
                  markDirty();
                }}
                className={ADD_BTN}
              >
            ...
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                ...
                onClick={() => {
                  setRelations([
                    ...relations,
                    { related_text: "", type: "other", use_picker: false },
                  ]);
                  markDirty();
                }}
                className={ADD_BTN}
            ...
            <div className="space-y-2">
              {relations.map((rel, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <label className="flex shrink-0 cursor-pointer items-center gap-1 text-xs text-text-tertiary">
                    <input
                      type="checkbox"
                      checked={rel.use_picker}
                      onChange={(e) => {
                        const n = [...relations];
                        n[idx] = {
                          ...n[idx]!,
                          use_picker: e.target.checked,
                  ...
                  {rel.use_picker ? (
                    <select
                      className={cn(INPUT, "flex-1")}
                      value={rel.related_person_id ?? ""}
                      onChange={(e) => {
                        const n = [...relations];
                        n[idx] = { ...n[idx]!, related_person_id: e.target.value || undefined };
                        setRelations(n);
                        markDirty();
                      }}
                    >
                    ...
                    <input
                      className={cn(INPUT, "flex-1")}
                      value={rel.related_text}
                      onChange={(e) => {
                        const n = [...relations];
                        n[idx] = { ...n[idx]!, related_text: e.target.value };
                        setRelations(n);
                        markDirty();
                      }}
          ...
          <section id="section-skills">
            ...
            <div className="mb-2 flex flex-wrap gap-1.5">
              {skills.map((s, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-1 rounded-md border border-border-default bg-surface-raised px-2 py-0.5 text-sm text-text-primary"
                >
                  {s.name}
                  <button
                    type="button"
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    if (val && !skills.some((s) => s.name === val)) {
          ...
          <section id="section-cadence">
            ...
            <p className="mb-3 text-xs text-text-tertiary">
              Set how often you want to stay in touch. Atlas will show this person on your follow-up
              list when the cadence period has passed.
            </p>
              ...
              {CADENCE_PRESETS.map((preset) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setCadencePreset(preset.value as number | null);
                      markDirty();
                    }}
                    className={cn(
            ...
            {cadencePreset === -1 && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  value={cadenceCustom}
                  onChange={(e) => {
                    setCadenceCustom(e.target.value);
                    markDirty();
                  }}
                  placeholder="Days"
                  min={1}
          ...
          <section id="section-interests">
              ...
              {interests.map((i, idx) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setInterests(interests.filter((_, ii) => ii !== idx));
                      markDirty();
                    }}
                    className="text-text-disabled hover:text-accent-danger"
                  >
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    if (val && !interests.some((i) => i.name === val)) {
                      setInterests([...interests, { name: val }]);
                      markDirty();
                    }
                    setInterestInput("");
                  }
              ...
              <datalist id="interests-autocomplete">
                {allInterests
                  .filter((i) => i.toLowerCase().includes(interestInput.toLowerCase()))
                  .map((i) => (
      ...
      {/* TOC sidebar */}
      <aside className="lg:flex hidden w-40 shrink-0 flex-col overflow-y-auto py-6 pr-4">
...
const ADD_BTN =
  "flex items-center gap-1 text-xs text-text-tertiary hover:text-text-primary transition-colors";
