...
import { recomputeAndPersist } from "@/core/people/last-contact";
import {
  PersonCreateSchema,
  PersonUpdateSchema,
  PersonEmailSchema,
...
let parsePhoneNumber: ((number: string, region?: string) => { number: string } | undefined) | null =
  null;
...
export const peopleRouter = router({
    ...
    .query(async ({ ctx, input }) => {
      const person = await db.person.findFirst({
        ...
        include: {
          ...
          organizations: {
            where: { deleted_at: null },
            orderBy: [{ is_primary: "desc" }, { is_current: "desc" }, { created_at: "asc" }],
          },
  ...
  create: protectedProcedure.input(PersonCreateSchema).mutation(async ({ ctx, input }) => {
    ...
    const person = await db.person.create({
      data: {
        ...
        external_data:
          input.external_data !== undefined
            ? (input.external_data as Prisma.InputJsonValue)
            : undefined,
      },
    });
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const person = await db.person.update({
        ...
        data: {
          ...
          ...(data.honorific_prefix !== undefined
          ...
          ...(data.honorific_suffix !== undefined
          ...
          ...(data.external_data !== undefined
            ? {
                external_data:
                  data.external_data !== null
                    ? (data.external_data as Prisma.InputJsonValue)
                    : Prisma.DbNull,
              }
            : {}),
        },
  ...
  snoozeFollowUp: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const now = new Date();
      const snooze_days = Math.round(
        (snoozeUntil.getTime() - now.getTime()) / (24 * 60 * 60 * 1000),
      );
  ...
  followUpList: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const where = {
        ...
        OR: [{ followup_snooze_until: null }, { followup_snooze_until: { lte: now } }],
        ...(input.relationship_type ? { relationship_type: input.relationship_type } : {}),
        ...(input.tag_ids?.length
    ...
    create: protectedProcedure
      .input(
        z
          ...
          .refine((d) => new Date(d.occurred_at).getTime() <= Date.now() + 5 * 60 * 1000, {
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.source_capture_id) {
          ...
          if (!c)
            throw new TRPCError({ code: "BAD_REQUEST", message: "source_capture_id not found" });
        }
    ...
    update: protectedProcedure
      .input(
        z
          ...
          .refine(
            (d) =>
              !d.occurred_at || new Date(d.occurred_at).getTime() <= Date.now() + 5 * 60 * 1000,
            {
              message: "occurred_at cannot be more than 5 minutes in the future",
              path: ["occurred_at"],
            },
  ...
  emails: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonEmail",
          entity_id: id,
          action: "person_email_add",
          meta: { person_id: input.person_id },
        }).catch(() => {});
        return row;
      }),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.$transaction(async (tx) => {
          ...
          if (row.is_primary) {
            ...
            if (next)
              await tx.personEmail.update({ where: { id: next.id }, data: { is_primary: true } });
          }
        });
  ...
  phones: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonPhone",
          entity_id: id,
          action: "person_phone_add",
        }).catch(() => {});
        return row;
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const e164 = data.number
          ? await tryNormalizePhone(data.number, ctx.user.locale_preset)
          : undefined;
        ...
        const updated = await db.$transaction(async (tx) => {
          ...
          return tx.personPhone.update({
            where: { id },
            data: { ...data, ...(e164 !== undefined ? { e164_normalized: e164 } : {}) },
          });
        });
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.$transaction(async (tx) => {
          ...
          if (row.is_primary) {
            ...
            if (next)
              await tx.personPhone.update({ where: { id: next.id }, data: { is_primary: true } });
          }
        });
  ...
  addresses: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonAddress",
          entity_id: id,
          action: "person_address_add",
        }).catch(() => {});
        return row;
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { id, ...data } = input;
        const row = await db.personAddress.findFirst({
          where: { id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true, person_id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.$transaction(async (tx) => {
          ...
          if (row.is_primary) {
            const next = await tx.personAddress.findFirst({
              where: { person_id: row.person_id, deleted_at: null, id: { not: input.id } },
              orderBy: { updated_at: "desc" },
            });
            if (next)
              await tx.personAddress.update({ where: { id: next.id }, data: { is_primary: true } });
          }
        });
  ...
  organizations: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const row = await db.$transaction(async (tx) => {
          ...
          return tx.personOrganization.create({
            data: {
              id,
              person_id: input.person_id,
              name: input.name,
              ...
              end_date: !input.is_current && input.end_date ? new Date(input.end_date) : undefined,
              source: input.source,
              source_id: input.source_id,
            },
          });
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonOrganization",
          entity_id: id,
          action: "person_organization_add",
        }).catch(() => {});
        return row;
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { id, ...data } = input;
        const row = await db.personOrganization.findFirst({
          where: { id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true, person_id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        ...
        const updated = await db.$transaction(async (tx) => {
          ...
          return tx.personOrganization.update({
            where: { id },
            data: {
              ...data,
        ...
        await logActivity({
          ...
          entity_type: "PersonOrganization",
          entity_id: id,
          action: "person_organization_update",
        }).catch(() => {});
        return updated;
      }),
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.$transaction(async (tx) => {
          ...
          if (row.is_primary) {
            const next = await tx.personOrganization.findFirst({
              where: { person_id: row.person_id, deleted_at: null, id: { not: input.id } },
              orderBy: { updated_at: "desc" },
            });
            if (next)
        ...
        await logActivity({
          ...
          entity_type: "PersonOrganization",
          entity_id: input.id,
          action: "person_organization_remove",
        }).catch(() => {});
        return { ok: true };
      }),
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          ...
          entity_type: "PersonOrganization",
          entity_id: input.id,
          action: "person_organization_restore",
        }).catch(() => {});
        return { ok: true };
      }),
  }),
  ...
  urls: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const row = await db.personUrl.create({
          data: {
            id,
            person_id: input.person_id,
            url: normalizedUrl,
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonUrl",
          entity_id: id,
          action: "person_url_add",
        }).catch(() => {});
        return row;
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { id, ...data } = input;
        const row = await db.personUrl.findFirst({
          where: { id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        ...
        const type =
          normalizedUrl && (!data.type || data.type === "other")
            ? detectUrlType(normalizedUrl)
            : data.type;

        const updated = await db.personUrl.update({
          where: { id },
          data: {
            ...data,
  ...
  events: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.$transaction(async (tx) => {
          if (input.type === "birthday") {
            await tx.personEvent.updateMany({
              where: { person_id: input.person_id, type: "birthday", deleted_at: null },
              data: { deleted_at: new Date() },
            });
          }
          return tx.personEvent.create({
            data: {
              id,
              person_id: input.person_id,
              type: input.type,
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonEvent",
          entity_id: id,
          action: "person_event_add",
        }).catch(() => {});
        return { id };
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { id, ...data } = input;
        const row = await db.personEvent.findFirst({
          where: { id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        const updated = await db.personEvent.update({
          where: { id },
  ...
  relations: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const id = newId();
        const row = await db.personRelation.create({
          data: {
            id,
            person_id: input.person_id,
            related_person_id: input.related_person_id,
            related_text: input.related_text,
            type: input.type,
            source: input.source,
            source_id: input.source_id,
          },
        ...
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonRelation",
          entity_id: id,
          action: "person_relation_add",
        }).catch(() => {});
        return row;
      }),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { id, ...data } = input;
        const row = await db.personRelation.findFirst({
          where: { id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        await db.personRelation.update({
          where: { id: input.id },
          data: { deleted_at: new Date() },
        });
        await logActivity({
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          ...
          entity_type: "PersonRelation",
          entity_id: input.id,
          action: "person_relation_restore",
        }).catch(() => {});
        return { ok: true };
      }),
  }),
  ...
  skills: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!person) throw new TRPCError({ code: "NOT_FOUND" });
        const id = newId();
        const row = await db.personSkill.create({
          data: { id, person_id: input.person_id, name: input.name },
        });
        await logActivity({
          user_id: ctx.user.id,
          entity_type: "PersonSkill",
          entity_id: id,
          action: "person_skill_add",
        }).catch(() => {});
        return row;
      }),
    ...
    remove: protectedProcedure
      .input(z.object({ id: z.string().uuid() }))
      .mutation(async ({ ctx, input }) => {
        const row = await db.personSkill.findFirst({
          where: { id: input.id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        const updated = await db.personSkill.update({
          where: { id: input.id },
  ...
  interests: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!person) throw new TRPCError({ code: "NOT_FOUND" });
        const id = newId();
        const row = await db.personInterest.create({
          data: { id, person_id: input.person_id, name: input.name },
        });
        await logActivity({
          user_id: ctx.user.id,
    ...
    update: protectedProcedure
      .input(z.object({ id: z.string().uuid(), name: z.string().min(1).max(200) }))
      .mutation(async ({ ctx, input }) => {
        const row = await db.personInterest.findFirst({
          where: { id: input.id, person: { user_id: ctx.user.id }, deleted_at: null },
          select: { id: true },
        });
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        const updated = await db.personInterest.update({
          where: { id: input.id },
          data: { name: input.name },
        });
        await logActivity({
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!row) throw new TRPCError({ code: "NOT_FOUND" });
        await db.personInterest.update({
          where: { id: input.id },
          data: { deleted_at: new Date() },
        });
        await logActivity({
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({
          ...
          entity_type: "PersonInterest",
          entity_id: input.id,
          action: "person_interest_restore",
        }).catch(() => {});
        return { ok: true };
      }),
  ...
  tags: router({
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          await db.tagOnPerson.create({
            data: { person_id: input.person_id, tag_id: input.tag_id },
          });
          await db.tag.update({
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existing = await db.tagOnPerson.findUnique({
          where: { person_id_tag_id: { person_id: input.person_id, tag_id: input.tag_id } },
        });
        if (existing) {
          await db.tagOnPerson.delete({
            where: { person_id_tag_id: { person_id: input.person_id, tag_id: input.tag_id } },
          });
          await db.tag
            .update({ where: { id: input.tag_id }, data: { usage_count: { decrement: 1 } } })
            .catch(() => {});
        }
        return { ok: true };
      }),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.tag_ids.length > 0) {
          ...
          if (unauthorized.length > 0)
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "One or more tag_ids do not belong to the current user",
            });
        }
        ...
        await db.$transaction([
          ...toAdd.map((tag_id) =>
            db.tagOnPerson.create({ data: { person_id: input.person_id, tag_id } }),
          ),
          ...toRemove.map((tag_id) =>
            db.tagOnPerson.delete({
              where: { person_id_tag_id: { person_id: input.person_id, tag_id } },
            }),
          ),
          ...toAdd.map((tag_id) =>
            db.tag.update({ where: { id: tag_id }, data: { usage_count: { increment: 1 } } }),
          ),
          ...toRemove.map((tag_id) =>
            db.tag.update({ where: { id: tag_id }, data: { usage_count: { decrement: 1 } } }),
          ),
        ]);
  ...
  search: protectedProcedure
    .input(
      z.object({ query: z.string().max(500), limit: z.number().int().min(1).max(20).default(8) }),
    )
    .query(async ({ ctx, input }) => {
          ...
          ...(q
            ? {
                OR: [
                  ...
                  {
                    emails: {
                      some: { email: { contains: q, mode: "insensitive" }, deleted_at: null },
                    },
                  },
                ],
              }
