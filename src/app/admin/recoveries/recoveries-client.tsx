                ...
                <div className="min-w-0 flex-1">
                  ...
                  {recovered && (
                    <p className="mt-0.5 font-mono text-2xs text-emerald-400/70">
                      Recovered: {recovered.tasks ?? 0}T · {recovered.projects ?? 0}P · {recovered.notes ?? 0}N
                      {totalRecovered !== null && ` (${totalRecovered} items total)`}
                    </p>
                  )}
                  <p className="font-mono text-2xs text-white/25 mt-1">{formatDate(r.created_at)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <FlagButton recoveryId={r.id} isFlagged={r.isFlagged} />
                  <ChevronRight size={16} className="text-white/20 transition-colors group-hover:text-white/50" />
                </div>
              </Link>
            );
          })}
        ...
        <div className="flex flex-col gap-2">
          {possibleOrphans.map((o) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="font-mono text-2xs text-white/25">{new Date(o.deleted_at!).toLocaleDateString()}</span>
                <ChevronRight size={16} className="text-amber-600/50 transition-colors group-hover:text-amber-400" />
              </div>
            </Link>
          ))}
        </div>
